...
use crate::ipc::{self, CmdMessage};
use crate::schema::controls_capnp::control_command;
...
pub fn send_arm(pub_: &dyn CmdPublisher) -> Result<()> {
    ...
    pub_.send_envelope(ipc::pack::<{ ipc::CMD_BUF }>(&msg)?)
}
...
pub fn send_takeoff(pub_: &dyn CmdPublisher, altitude_m: f64) -> Result<()> {
    let mut msg = capnp::message::Builder::new_default();
    {
        let mut cmd = msg.init_root::<control_command::Builder<'_>>();
        cmd.set_id(next_cmd_id());
        cmd.init_takeoff().set_altitude_m(altitude_m);
    }
    pub_.send_envelope(ipc::pack::<{ ipc::CMD_BUF }>(&msg)?)
}

pub fn send_goto(
    pub_: &dyn CmdPublisher,
    lat: f64,
    lon: f64,
    alt: f64,
    speed: f64,
...
pub fn send_return_home(pub_: &dyn CmdPublisher, altitude_m: f64) -> Result<()> {
    let mut msg = capnp::message::Builder::new_default();
    {
        let mut cmd = msg.init_root::<control_command::Builder<'_>>();
        cmd.set_id(next_cmd_id());
        cmd.init_return_home().set_altitude_m(altitude_m);
    }
    pub_.send_envelope(ipc::pack::<{ ipc::CMD_BUF }>(&msg)?)
}
...
pub fn send_land(pub_: &dyn CmdPublisher, descent_speed_ms: f64) -> Result<()> {
    let mut msg = capnp::message::Builder::new_default();
    {
        let mut cmd = msg.init_root::<control_command::Builder<'_>>();
        cmd.set_id(next_cmd_id());
        cmd.init_land().set_descent_speed_ms(descent_speed_ms);
    }
    pub_.send_envelope(ipc::pack::<{ ipc::CMD_BUF }>(&msg)?)
}
